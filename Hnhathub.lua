...
-- ========================================================================
local Theme = {
    -- Backgrounds (Deep dark với tone xanh nhẹ)
    ...
    -- Accent (Orange gradient premium)
...
-- 🖋️ Premium font system
local Fonts = {
    Display = Enum.Font.GothamBold,      -- Headlines
    Title = Enum.Font.GothamBold,        -- Titles
    Header = Enum.Font.GothamSemibold,   -- Sub-headers
    Body = Enum.Font.Gotham,             -- Body text
    Caption = Enum.Font.Gotham,          -- Small text
    Mono = Enum.Font.Code,               -- Code
    Icon = Enum.Font.GothamBold,
}

-- ⚡ Premium tween presets
local Tweens = {
...
-- 🎭 Icon system (Unicode symbols - đẹp và clean)
local Icons = {
    -- Navigation
    Home = "⌂",
    Dashboard = "◈",
...
-- 🌟 Shadow effect cao cấp (dùng ImageLabel với shadow asset)
...
-- 💫 Glow effect (dùng cho accent elements)
...
local function createRipple(parent, x, y, color)
    local ripple = create("Frame", {
        ...
        AnchorPoint = Vector2.new(0.5, 0.5),
        BackgroundColor3 = color or Theme.AccentSoft,
...
-- ✨ Particle system (hiệu ứng sao lấp lánh)
...
function Library:Destroy()
    ...
    if self.Blur then 
        ...
        task.delay(0.4, function() pcall(function() self.Blur:Destroy() end) end)
        self.Blur = nil 
    end
...
function Library:EnsureNotifications()
    ...
    local holder = create("Frame", {
        ...
        Position = UDim2.new(1, -20, 0, 20), Size = UDim2.fromOffset(340, 0), 
...
function Library:Notify(data)
    ...
    local contentHolder = create("Frame", {
        Parent = notification, Size = UDim2.new(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y, BackgroundTransparency = 1,
    })
    addList(contentHolder, 8)

    -- Accent left bar với gradient
    local accentBar = create("Frame", { 
        Parent = notification, Size = UDim2.new(0, 3, 1, -4), 
    ...
    local headerRow = create("Frame", {
        Parent = contentHolder, Size = UDim2.new(1, 0, 0, 22),
        BackgroundTransparency = 1,
    })
    
    -- Icon circle
    local iconCircle = create("Frame", {
        Parent = headerRow, Size = UDim2.fromOffset(22, 22),
        Position = UDim2.new(0, 0, 0, 0),
        BackgroundColor3 = accent, BackgroundTransparency = 0.85,
        BorderSizePixel = 0,
    })
    addCorner(iconCircle, 999)
    addStroke(iconCircle, accent, 1, 0.5)
    
    create("TextLabel", {
        Parent = iconCircle, Size = UDim2.fromScale(1, 1),
        BackgroundTransparency = 1, Text = config.icon,
        TextColor3 = accent, TextSize = 13, Font = Fonts.Icon,
    ...
    local titleLabel = create("TextLabel", {
        ...
        Text = title, TextColor3 = Theme.Text, TextTransparency = 1, 
        TextSize = 14, Font = Fonts.Title,
    ...
    local contentLabel = create("TextLabel", {
        ...
        Text = content, TextColor3 = Theme.TextSecondary, TextTransparency = 1, 
        TextWrapped = true, TextSize = 12, Font = Fonts.Body, 
        TextXAlignment = Enum.TextXAlignment.Left, 
        TextYAlignment = Enum.TextYAlignment.Top,
    })

    -- Progress bar với gradient
    local durationTrack = create("Frame", {
        Parent = contentHolder, Size = UDim2.new(1, 0, 0, 3), 
    ...
    local fill = create("Frame", { 
        Parent = durationTrack, Size = UDim2.new(1, 0, 1, 0), 
        BackgroundColor3 = accent, BorderSizePixel = 0 
    })
    addCorner(fill, 99)
    addGradient(fill, ColorSequence.new(accent, Theme.AccentSoft), 90)

    -- Slide-in animation với blur
    ...
    task.wait(0.05)
    tween(titleLabel, { TextTransparency = 0 }, Tweens.Smooth)
    task.wait(0.05)
    tween(contentLabel, { TextTransparency = 0 }, Tweens.Smooth)
...
local function createCard(parent, fixedHeight)
    local card = create("Frame", {
        ...
        AutomaticSize = fixedHeight and Enum.AutomaticSize.None or Enum.AutomaticSize.Y,
...
function Library:CreateWindow(options)
    ...
    local sidebar = create("Frame", { 
        Parent = root, Name = "Sidebar", Size = UDim2.new(0, 210, 1, 0), 
        BackgroundColor3 = Theme.Panel, BackgroundTransparency = 0.15,
    ...
    -- Divider với glow gradient
    local divider = create("Frame", { 
        Parent = root, Name = "Divider", Position = UDim2.new(0, 210, 0, 0), 
        Size = UDim2.new(0, 1, 1, 0), BackgroundColor3 = Theme.BorderLight, 
        BorderSizePixel = 0, BackgroundTransparency = 0.3, ZIndex = 11 
    })
    ...
    local brandHolder = create("Frame", { 
        ...
        Size = UDim2.new(1, 0, 0, 90), ZIndex = 12 
    })
    addPadding(brandHolder, 18, 18, 18, 15)

    -- Logo circle với glow
    local logoCircle = create("Frame", {
        Parent = brandHolder, Size = UDim2.fromOffset(36, 36),
        Position = UDim2.new(0, 0, 0, 0),
        BackgroundColor3 = Theme.Accent, BorderSizePixel = 0,
        ZIndex = 13,
    })
    addCorner(logoCircle, 12)
    addGradient(logoCircle, ColorSequence.new(Theme.Accent, Theme.AccentDark), 135)
    addGlow(logoCircle, Theme.Accent, 0.5, 15)
    
    create("TextLabel", {
        Parent = logoCircle, Size = UDim2.fromScale(1, 1),
        BackgroundTransparency = 1, Text = "S",
        TextColor3 = Theme.Background, TextSize = 20, Font = Fonts.Display,
        ZIndex = 14,
    })

    local brandTitle = create("TextLabel", {
        Parent = brandHolder, BackgroundTransparency = 1, 
        Position = UDim2.new(0, 46, 0, 2),
        Size = UDim2.new(1, -46, 0, 22), 
        Text = options.Name or "Saloi Hub", TextColor3 = Theme.Text, TextSize = 18, 
        Font = Fonts.Display, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 12
    })
    ...
    local brandSubtitle = create("TextLabel", {
        ...
        TextColor3 = Theme.AccentSoft, TextSize = 11, Font = Fonts.Header, 
        TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 12
    })
    
    -- Separator line dưới brand
    local brandSeparator = create("Frame", {
        Parent = brandHolder, Position = UDim2.new(0, 0, 1, -2),
        Size = UDim2.new(1, 0, 0, 1), BackgroundColor3 = Theme.Border,
        BorderSizePixel = 0, BackgroundTransparency = 0.5, ZIndex = 12,
    })
    addGradient(brandSeparator, ColorSequence.new({
    ...
    -- 📋 Tab scroller với custom style
    local tabScroller = create("ScrollingFrame", {
        Parent = sidebar, Name = "TabScroller", Position = UDim2.new(0, 0, 0, 90), 
        Size = UDim2.new(1, 0, 1, -130), BackgroundTransparency = 1, BorderSizePixel = 0, 
    ...
    self:Connect(tabLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function() 
        setCanvasToLayout(tabScroller, tabLayout, 15) 
    end)
    ...
    local userFooter = create("Frame", {
        ...
        BackgroundColor3 = Theme.Surface, BackgroundTransparency = 0.4,
        BorderSizePixel = 0, ZIndex = 12,
    })
    addPadding(userFooter, 15, 15, 10, 10)
    
    local userSeparator = create("Frame", {
        Parent = userFooter, Position = UDim2.new(0, 0, 0, 0),
        Size = UDim2.new(1, 0, 0, 1), BackgroundColor3 = Theme.Border,
        BorderSizePixel = 0, BackgroundTransparency = 0.5, ZIndex = 12,
    })
    
    local userAvatar = create("Frame", {
        Parent = userFooter, Size = UDim2.fromOffset(30, 30),
        Position = UDim2.new(0, 0, 0.5, -15),
        BackgroundColor3 = Theme.Accent, BorderSizePixel = 0, ZIndex = 13,
    })
    addCorner(userAvatar, 999)
    addGradient(userAvatar, ColorSequence.new(Theme.Accent, Theme.AccentDark), 135)
    ...
    task.spawn(function()
        pcall(function()
            ...
            local img = create("ImageLabel", {
                Parent = userAvatar, Size = UDim2.fromScale(1, 1),
                BackgroundTransparency = 1, Image = thumb, ZIndex = 14,
            })
            addCorner(img, 999)
        end)
    end)
    ...
    local onlineDot = create("Frame", {
        Parent = userAvatar, Size = UDim2.fromOffset(9, 9),
        Position = UDim2.new(1, -8, 1, -8),
        BackgroundColor3 = Theme.Success, BorderSizePixel = 0, ZIndex = 15,
    })
    addCorner(onlineDot, 999)
    addStroke(onlineDot, Theme.Panel, 2, 0)
    
    create("TextLabel", {
        Parent = userFooter, BackgroundTransparency = 1,
        Position = UDim2.new(0, 40, 0, 4), Size = UDim2.new(1, -40, 0, 14),
        Text = LocalPlayer and LocalPlayer.DisplayName or "Unknown",
        TextColor3 = Theme.Text, TextSize = 12, Font = Fonts.Header,
        TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 13,
    })
    create("TextLabel", {
        Parent = userFooter, BackgroundTransparency = 1,
        Position = UDim2.new(0, 40, 0, 19), Size = UDim2.new(1, -40, 0, 12),
        Text = "@" .. (LocalPlayer and LocalPlayer.Name or "player"),
        TextColor3 = Theme.Muted, TextSize = 10, Font = Fonts.Body,
        TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 13,
    })

    -- 📄 Content area
    local contentHolder = create("Frame", { 
        Parent = root, Name = "ContentHolder", Position = UDim2.new(0, 211, 0, 0), 
        Size = UDim2.new(1, -211, 1, 0), BackgroundTransparency = 1, BorderSizePixel = 0, ZIndex = 11 
    })

    -- 🎯 Premium top bar
    local topBar = create("Frame", { 
        Parent = contentHolder, Name = "TopBar", Size = UDim2.new(1, 0, 0, 65), 
        BackgroundColor3 = Theme.Surface, BackgroundTransparency = 0.4,
        BorderSizePixel = 0, ZIndex = 12 
    })
    ...
    -- Breadcrumb icon
    local breadcrumbIcon = create("TextLabel", {
        Parent = topBar, BackgroundTransparency = 1,
        Position = UDim2.new(0, 0, 0, 3), Size = UDim2.fromOffset(22, 22),
        Text = Icons.Diamond, TextColor3 = Theme.Accent, TextSize = 16,
        Font = Fonts.Icon, ZIndex = 13,
    })

    local headerTitle = create("TextLabel", {
        Parent = topBar, BackgroundTransparency = 1, 
        Position = UDim2.new(0, 28, 0, 0), Size = UDim2.new(1, -130, 0, 26), 
        Text = options.Name or "Saloi Hub", TextColor3 = Theme.Text, TextSize = 20, 
        Font = Fonts.Display, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 13
    })

    local headerSubtitle = create("TextLabel", {
        Parent = topBar, BackgroundTransparency = 1, 
        Position = UDim2.new(0, 28, 0, 28), Size = UDim2.new(1, -130, 0, 16), 
        Text = options.Subtitle or "Premium UI", 
    ...
    local function createControlButton(parentTopBar, icon, position, hoverColor)
        local btn = create("TextButton", {
            Parent = parentTopBar, AnchorPoint = Vector2.new(1, 0), 
            Position = position, Size = UDim2.fromOffset(32, 32), 
            BackgroundColor3 = Theme.Inline, BackgroundTransparency = 0.3,
            BorderSizePixel = 0, AutoButtonColor = false, 
            Text = icon, TextColor3 = Theme.TextSecondary, TextSize = 14, 
            Font = Fonts.Icon, ZIndex = 14
        })
        addCorner(btn, 8)
        ...
        btn.MouseEnter:Connect(function()
            tween(btn, { 
                BackgroundColor3 = hoverColor or Theme.InlineHover,
    ...
    local pagesHolder = create("Frame", { 
        Parent = contentHolder, Name = "PagesHolder", Position = UDim2.new(0, 0, 0, 65), 
        Size = UDim2.new(1, 0, 1, -65), BackgroundTransparency = 1, BorderSizePixel = 0, ZIndex = 12 
    })
    ...
    local openButton = create("TextButton", {
        Parent = rootGui, Name = "OpenLogoButton",
        Size = UDim2.fromOffset(55, 55),
        Position = UDim2.new(0, 25, 0, 25),
        BackgroundColor3 = Theme.Surface, AutoButtonColor = false,
        Visible = false, Text = "", ZIndex = 9999,
    })
    addCorner(openButton, 16)
    addStroke(openButton, Theme.Accent, 2, 0)
    addShadow(openButton, 0.5, 40, Vector2.new(0, 8))
    
    -- Gradient background
    addGradient(openButton, ColorSequence.new({
    ...
    local logoImage = create("ImageLabel", {
        Parent = openButton, Size = UDim2.fromScale(1, 1),
        BackgroundTransparency = 1, Image = "", ZIndex = 10000,
    })
    addCorner(logoImage, 16)
    
    -- Fallback "S" logo với gradient
    local fallbackLogo = create("TextLabel", {
        Parent = openButton, Size = UDim2.fromScale(1, 1),
        BackgroundTransparency = 1, Text = "S",
        TextColor3 = Theme.Accent, TextSize = 26, Font = Fonts.Display,
        ZIndex = 10000,
    })
    ...
    openButton.MouseEnter:Connect(function() 
        tween(openButton, { Size = UDim2.fromOffset(62, 62) }, Tweens.Bounce)
        tween(logoGlow, { ImageTransparency = 0.2, Size = UDim2.new(1, 40, 1, 40) }, Tweens.Smooth)
    end)
    openButton.MouseLeave:Connect(function() 
        tween(openButton, { Size = UDim2.fromOffset(55, 55) }, Tweens.Bounce)
        tween(logoGlow, { ImageTransparency = 0.6, Size = UDim2.new(1, 25, 1, 25) }, Tweens.Smooth)
    end)
...
-- 🎯 Premium tab selection với smooth transition
function WindowClass:SelectTab(targetTab)
    for _, tab in ipairs(self.Tabs) do
        ...
        if isActive then
            ...
            for _, child in ipairs(tab.Page:GetChildren()) do
                if child:IsA("Frame") or child:IsA("TextLabel") then
                    child.Position = child.Position + UDim2.new(0.01, 0, 0, 0)
                end
            end
            tween(tab.Page, { Position = UDim2.new(0, 0, 0, 0) }, Tweens.Smooth)
        else
            task.delay(0.15, function()
                if self.ActiveTab ~= tab then tab.Page.Visible = false end
            end)
        end
        ...
        tween(tab.ButtonStroke, { 
            Transparency = isActive and 0 or 0.4, 
            Color = isActive and Theme.AccentSoft or Theme.Border 
        }, Tweens.Smooth)
        tween(tab.ButtonLabel, { 
            TextColor3 = isActive and Theme.Background or Theme.TextSecondary 
        }, Tweens.Smooth)
        if tab.IconLabel then
            tween(tab.IconLabel, { 
                TextColor3 = isActive and Theme.Background or Theme.Accent 
            }, Tweens.Smooth)
        end
...
function WindowClass:CreateTab(title, icon)
    ...
    local tabButton = create("TextButton", { 
        Parent = self.TabScroller, Size = UDim2.new(1, 0, 0, 42), 
    ...
    local indicator = create("Frame", {
        Parent = tabButton, Position = UDim2.new(0, 0, 0.2, 0), 
    ...
    addGradient(indicator, ColorSequence.new(Theme.Accent, Theme.AccentSoft), 90)

    -- Icon
    local iconLabel = create("TextLabel", {
        Parent = tabButton, BackgroundTransparency = 1,
        Position = UDim2.new(0, 12, 0, 0), Size = UDim2.new(0, 24, 1, 0),
        Text = icon, TextColor3 = Theme.Accent, TextSize = 15,
        Font = Fonts.Icon, ZIndex = 14,
    })
    ...
    local tabButtonLabel = create("TextLabel", {
        ...
        Text = title, TextColor3 = Theme.TextSecondary, TextSize = 13, 
        Font = Fonts.Header, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 14
    })

    -- Page với custom scrollbar
    ...
    self.Library:Connect(pageLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function() 
        setCanvasToLayout(page, pageLayout, 20) 
    end)

    local tab = setmetatable({ 
        Window = self, Title = title, Icon = icon,
        Button = tabButton, ButtonStroke = tabButtonStroke, 
        ButtonLabel = tabButtonLabel, IconLabel = iconLabel,
        Page = page, Layout = pageLayout, Indicator = indicator 
    }, TabClass)
    ...
    tabButton.MouseEnter:Connect(function() 
        if self.ActiveTab ~= tab then 
            tween(tabButton, { 
                BackgroundColor3 = Theme.Inline, BackgroundTransparency = 0,
            }, Tweens.Fast)
            tween(tabButtonLabel, { 
                TextColor3 = Theme.Text,
                Position = UDim2.new(0, 44, 0, 0),
            }, Tweens.Fast)
            tween(iconLabel, { Position = UDim2.new(0, 14, 0, 0) }, Tweens.Fast)
        end 
    end)
    tabButton.MouseLeave:Connect(function() 
        if self.ActiveTab ~= tab then 
            ...
            tween(iconLabel, { Position = UDim2.new(0, 12, 0, 0) }, Tweens.Fast)
        end 
    end)
...
-- 📌 Premium section với icon
function TabClass:CreateSection(text, icon)
    icon = icon or Icons.Diamond
    
    local holder = create("Frame", {
        Parent = self.Page, BackgroundTransparency = 1, 
        Size = UDim2.new(1, 0, 0, 32), ZIndex = 14
    })
    
    local iconLabel = create("TextLabel", {
        Parent = holder, BackgroundTransparency = 1,
        Position = UDim2.new(0, 0, 0, 0), Size = UDim2.fromOffset(18, 100),
        Text = icon, TextColor3 = Theme.Accent, TextSize = 13,
        Font = Fonts.Icon, TextYAlignment = Enum.TextYAlignment.Bottom, ZIndex = 15,
    })
    ...
    addGradient(line, ColorSequence.new({
        ColorSequenceKeypoint.new(0, Theme.Accent),
        ColorSequenceKeypoint.new(0.4, Theme.Border),
        ColorSequenceKeypoint.new(1, Theme.Border),
    }), 0)
...
function TabClass:CreateParagraph(data)
    ...
    -- Top accent line với gradient
    local accentBar = create("Frame", { 
        Parent = card, Size = UDim2.new(0, 50, 0, 3), 
    ...
    create("TextLabel", { 
        ...
        TextColor3 = Theme.Text, TextSize = 16, Font = Fonts.Display, TextWrapped = true, 
    ...
    create("TextLabel", { 
        ...
        TextColor3 = Theme.TextSecondary, TextSize = 12, Font = Fonts.Body, TextWrapped = true, 
        TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Top, ZIndex = 15 
    })
    return card
end
...
function TabClass:CreateLabel(text)
    ...
    local iconDot = create("Frame", {
        Parent = card, Position = UDim2.new(0, 0, 0, 5),
    ...
    create("TextLabel", { 
        ...
        AutomaticSize = Enum.AutomaticSize.Y, Text = text or "", TextColor3 = Theme.TextSecondary, 
...
function TabClass:CreateButton(data)
    ...
    local iconContainer = create("Frame", {
        Parent = card, Position = UDim2.new(0, 12, 0.5, -14),
        Size = UDim2.fromOffset(28, 28), BackgroundColor3 = Theme.Accent,
        BackgroundTransparency = 0.85, BorderSizePixel = 0, ZIndex = 15,
    })
    addCorner(iconContainer, 8)
    addStroke(iconContainer, Theme.Accent, 1, 0.5)
    
    local iconLabel = create("TextLabel", {
        Parent = iconContainer, Size = UDim2.fromScale(1, 1),
        BackgroundTransparency = 1, Text = data.Icon or Icons.Play,
        TextColor3 = Theme.Accent, TextSize = 13, Font = Fonts.Icon, ZIndex = 16,
    })
    
    local label = create("TextLabel", { 
        Parent = card, BackgroundTransparency = 1, Position = UDim2.new(0, 50, 0, 0), 
        Size = UDim2.new(1, -85, 1, 0), Text = data.Name or "Button", TextColor3 = Theme.Text, 
        TextSize = 13, Font = Fonts.Header, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 15 
    })
    
    local arrow = create("TextLabel", { 
        Parent = card, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(1, -14, 0.5, 0), 
        Size = UDim2.fromOffset(20, 20), BackgroundTransparency = 1, Text = Icons.Arrow, 
        TextColor3 = Theme.Accent, TextSize = 20, Font = Fonts.Header, ZIndex = 15 
    })

    button.MouseEnter:Connect(function() 
        tween(card, { BackgroundColor3 = Theme.SurfaceHover }, Tweens.Fast)
        tween(iconContainer, { BackgroundTransparency = 0.7 }, Tweens.Fast)
        tween(arrow, { Position = UDim2.new(1, -10, 0.5, 0), TextColor3 = Theme.AccentSoft }, Tweens.Fast)
    end)
    button.MouseLeave:Connect(function() 
        tween(card, { BackgroundColor3 = Theme.Surface }, Tweens.Fast)
        tween(iconContainer, { BackgroundTransparency = 0.85 }, Tweens.Fast)
        tween(arrow, { Position = UDim2.new(1, -14, 0.5, 0), TextColor3 = Theme.Accent }, Tweens.Fast)
    end)
...
function TabClass:CreateToggle(data)
    ...
    local card = createCard(self.Page, 54)
    card.ZIndex = 14
    ...
    local iconContainer = create("Frame", {
        ...
        BackgroundColor3 = currentValue and Theme.Success or Theme.Inline,
        BackgroundTransparency = 0.8, BorderSizePixel = 0, ZIndex = 15,
    })
    addCorner(iconContainer, 8)
    ...
    local iconText = create("TextLabel", {
        Parent = iconContainer, Size = UDim2.fromScale(1, 1),
        BackgroundTransparency = 1, 
        Text = data.Icon or Icons.Zap,
        TextColor3 = currentValue and Theme.Success or Theme.Muted, 
        TextSize = 13, Font = Fonts.Icon, ZIndex = 16,
    })
    
    create("TextLabel", { 
        Parent = card, BackgroundTransparency = 1, Position = UDim2.new(0, 50, 0, 9), 
        Size = UDim2.new(1, -110, 0, 18), Text = data.Name or "Toggle", TextColor3 = Theme.Text, 
        TextSize = 13, Font = Fonts.Header, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 15 
    })
    local stateLabel = create("TextLabel", { 
        Parent = card, BackgroundTransparency = 1, Position = UDim2.new(0, 50, 0, 28), 
    ...
    local track = create("Frame", { 
        ...
        BackgroundColor3 = currentValue and Theme.Accent or Theme.Inline, 
        BorderSizePixel = 0, ZIndex = 15 
    })
    addCorner(track, 99)
    if currentValue then
        addGradient(track, ColorSequence.new(Theme.Accent, Theme.AccentSoft), 0)
    end
    
    local knob = create("Frame", { 
        Parent = track, 
        Position = currentValue and UDim2.new(1, -21, 0.5, -9) or UDim2.new(0, 3, 0.5, -9), 
        Size = UDim2.fromOffset(18, 18), BackgroundColor3 = Theme.Text, 
    ...
    local function setValue(value, shouldTrigger)
        ...
        tween(iconStroke, { 
            Color = currentValue and Theme.Success or Theme.Border 
        }, Tweens.Normal)
        tween(iconText, { 
            TextColor3 = currentValue and Theme.Success or Theme.Muted 
        }, Tweens.Normal)
    ...
    clickArea.MouseButton1Click:Connect(function() 
        local mouse = UserInputService:GetMouseLocation()
        createRipple(card, mouse.X, mouse.Y)
        setValue(not currentValue, true) 
    end)
...
function TabClass:CreateInput(data)
    local card = createCard(self.Page, 80)
    card.ZIndex = 14
    
    -- Icon + Title row
    local iconContainer = create("Frame", {
        Parent = card, Position = UDim2.new(0, 12, 0, 10),
        Size = UDim2.fromOffset(20, 20), BackgroundTransparency = 1, ZIndex = 15,
    })
    create("TextLabel", {
        Parent = iconContainer, Size = UDim2.fromScale(1, 1),
        BackgroundTransparency = 1, Text = data.Icon or Icons.Search,
        TextColor3 = Theme.Accent, TextSize = 14, Font = Fonts.Icon, ZIndex = 15,
    })
    
    create("TextLabel", { 
        Parent = card, BackgroundTransparency = 1, Position = UDim2.new(0, 36, 0, 10), 
        Size = UDim2.new(1, -48, 0, 20), Text = data.Name or "Input", TextColor3 = Theme.Text, 
        TextSize = 13, Font = Fonts.Header, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 15 
    })
    
    local inputHolder = create("Frame", { 
        Parent = card, Position = UDim2.new(0, 12, 0, 36), Size = UDim2.new(1, -24, 0, 34), 
        BackgroundColor3 = Theme.Inline, BackgroundTransparency = 0.2,
        BorderSizePixel = 0, ZIndex = 15 
    })
    ...
    local textBox = create("TextBox", { 
        Parent = inputHolder, BackgroundTransparency = 1, Position = UDim2.new(0, 12, 0, 0), 
        Size = UDim2.new(1, -24, 1, 0), ClearTextOnFocus = false, Text = data.Default or "", 
        PlaceholderText = data.PlaceholderText or "Enter text...", TextColor3 = Theme.Text, 
        PlaceholderColor3 = Theme.Muted, TextSize = 13, Font = Fonts.Body, 
        TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 16 
    })

    textBox.Focused:Connect(function()
        tween(inputStroke, { Color = Theme.Accent, Transparency = 0 }, Tweens.Normal)
        tween(inputHolder, { BackgroundTransparency = 0 }, Tweens.Normal)
    end)
    
    textBox.FocusLost:Connect(function()
        tween(inputStroke, { Color = Theme.Border, Transparency = 0.4 }, Tweens.Normal)
        tween(inputHolder, { BackgroundTransparency = 0.2 }, Tweens.Normal)
...
function TabClass:CreateDropdown(data)
    ...
    create("TextLabel", {
        ...
        Size = UDim2.fromOffset(18, 18), Position = UDim2.new(0, 0, 0, 0),
        Text = data.Icon or Icons.Menu,
        TextColor3 = Theme.Accent, TextSize = 14, Font = Fonts.Icon, ZIndex = 15,
    })
    ...
    local selector = create("TextButton", { 
        Parent = card, Size = UDim2.new(1, 0, 0, 36), 
        BackgroundColor3 = Theme.Inline, BackgroundTransparency = 0.2,
    ...
    local selectorText = create("TextLabel", { 
        Parent = selector, BackgroundTransparency = 1, Position = UDim2.new(0, 12, 0, 0), 
        Size = UDim2.new(1, -36, 1, 0), Text = tostring(selected), TextColor3 = Theme.Text, 
        TextSize = 12, Font = Fonts.Body, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 16 
    })
    local arrow = create("TextLabel", { 
        Parent = selector, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(1, -12, 0.5, 0), 
        Size = UDim2.fromOffset(16, 16), BackgroundTransparency = 1, Text = Icons.ArrowDown, 
        TextColor3 = Theme.Accent, TextSize = 14, Font = Fonts.Header, ZIndex = 16 
    })
    ...
    local function rebuildButtons()
        ...
        for _, optionValue in ipairs(options) do
            ...
            local optionButton = create("TextButton", { 
                Parent = optionsHolder, Size = UDim2.new(1, 0, 0, 32), 
            ...
            addStroke(optionButton, isSelected and Theme.AccentSoft or Theme.Border, 1, isSelected and 0 or 0.5)
            
            local checkIcon = create("TextLabel", {
                Parent = optionButton, BackgroundTransparency = 1,
                Position = UDim2.new(0, 10, 0, 0), Size = UDim2.fromOffset(14, 32),
                Text = isSelected and Icons.Check or "•",
                TextColor3 = isSelected and Theme.Background or Theme.Muted,
                TextSize = isSelected and 12 or 16, Font = Fonts.Icon, ZIndex = 17,
            })
            ...
            local optionLabel = create("TextLabel", { 
                ...
                TextColor3 = isSelected and Theme.Background or Theme.TextSecondary, 
                TextSize = 12, Font = Fonts.Body, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 17 
            })

            optionButton.MouseEnter:Connect(function() 
                if not isSelected then 
                    tween(optionButton, { BackgroundColor3 = Theme.Inline, BackgroundTransparency = 0 }, Tweens.Fast) 
                end 
            end)
            optionButton.MouseLeave:Connect(function() 
                if not isSelected then 
                    tween(optionButton, { BackgroundColor3 = Theme.Surface, BackgroundTransparency = 0.3 }, Tweens.Fast) 
                end 
            end)
            optionButton.MouseButton1Click:Connect(function()
                ...
                if data.Flag then self.Window.Library.Flags[data.Flag] = selected end
                isOpen = false
                tween(arrow, { Rotation = 0 }, Tweens.Smooth)
    ...
    selector.MouseEnter:Connect(function() 
        tween(selectorStroke, { Color = Theme.Accent, Transparency = 0 }, Tweens.Fast) 
        tween(selector, { BackgroundTransparency = 0 }, Tweens.Fast)
    end)
    selector.MouseLeave:Connect(function() 
        if not isOpen then 
            tween(selectorStroke, { Color = Theme.Border, Transparency = 0.4 }, Tweens.Fast)
            tween(selector, { BackgroundTransparency = 0.2 }, Tweens.Fast)
        end
    end)
    selector.MouseButton1Click:Connect(function()
        ...
        tween(selectorStroke, { 
            ...
            Transparency = isOpen and 0 or 0.4 
        }, Tweens.Fast)
    end)
...
function TabClass:CreateSlider(data)
    ...
    local card = createCard(self.Page, 82)
    card.ZIndex = 14
    
    -- Icon + title + value row
    create("TextLabel", {
        Parent = card, BackgroundTransparency = 1,
        Position = UDim2.new(0, 12, 0, 10), Size = UDim2.fromOffset(18, 18),
        Text = data.Icon or Icons.Target,
        TextColor3 = Theme.Accent, TextSize = 14, Font = Fonts.Icon, ZIndex = 15,
    })
    
    create("TextLabel", { 
        Parent = card, BackgroundTransparency = 1, Position = UDim2.new(0, 36, 0, 10), 
        Size = UDim2.new(1, -110, 0, 20), Text = data.Name or "Slider", TextColor3 = Theme.Text, 
        TextSize = 13, Font = Fonts.Header, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 15 
    })
    
    -- Value badge
    local valueBadge = create("Frame", {
        Parent = card, Position = UDim2.new(1, -80, 0, 8), 
        Size = UDim2.fromOffset(68, 24),
        BackgroundColor3 = Theme.Accent, BackgroundTransparency = 0.85,
        BorderSizePixel = 0, ZIndex = 15,
    })
    addCorner(valueBadge, 8)
    addStroke(valueBadge, Theme.Accent, 1, 0.5)
    
    local valueLabel = create("TextLabel", { 
        Parent = valueBadge, BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1),
    ...
    -- Track
    local track = create("Frame", { 
        Parent = card, Position = UDim2.new(0, 14, 0, 48), Size = UDim2.new(1, -28, 0, 6), 
    ...
    addCorner(thumb, 999)
    addStroke(thumb, Theme.Accent, 2.5, 0)
    local thumbGlow = addGlow(thumb, Theme.Accent, 0.6, 12)
    
    -- Min/Max labels
    create("TextLabel", {
        Parent = card, BackgroundTransparency = 1,
        Position = UDim2.new(0, 14, 1, -18), Size = UDim2.new(0, 50, 0, 14),
        Text = formatNumber(minimum), TextColor3 = Theme.MutedDark,
        TextSize = 10, Font = Fonts.Body, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 15,
    })
    create("TextLabel", {
        Parent = card, BackgroundTransparency = 1,
        Position = UDim2.new(1, -64, 1, -18), Size = UDim2.new(0, 50, 0, 14),
        Text = formatNumber(maximum), TextColor3 = Theme.MutedDark,
        TextSize = 10, Font = Fonts.Body, TextXAlignment = Enum.TextXAlignment.Right, ZIndex = 15,
    })
    
    local dragZone = create("TextButton", { 
        Parent = track, Size = UDim2.new(1, 0, 1, 16), Position = UDim2.new(0, 0, 0, -8), 
...
HomeTab:CreateParagraph({
    Title = "◆ Welcome to Saloi Hub Premium",
    Content = "Giao diện đã được nâng cấp lên phiên bản Premium v3.0 với glassmorphism, particle effects, premium typography và hàng loạt micro-interactions. Bấm biểu tượng ─ để thu gọn, ✕ để ẩn về logo.",
})
...
local function loadModuleFromWeb(fileName, label)
    ...
    if not func then
        local msg = "Compile: " .. tostring(compileErr)
        Runtime.ModuleStatus[label] = msg
        return false, msg
    end
    ...
    if not runSuccess then
        local msg = "Runtime: " .. tostring(runErr)
        Runtime.ModuleStatus[label] = msg
        return false, msg
    end
...
for _, mod in ipairs(modules) do
    ...
    if ok then
        ...
        Hub.Helpers.Notify("Module Error", mod.label .. ": " .. tostring(message), 6, "error")
    end
end
...
HomeTab:CreateButton({
    ...
    Callback = function()
        Hub.Helpers.Notify(
            "Saloi Hub", 
            "Loaded " .. tostring(loadedCount) .. "/" .. tostring(#modules) .. " modules successfully.", 
            5, "success"
        )
    end,
})
...
Hub.Helpers.Notify(
    "Saloi Hub Premium", 
    "System loaded " .. tostring(loadedCount) .. "/" .. tostring(#modules) .. " modules.", 
    5, "success"
)
