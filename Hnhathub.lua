...
local function makeDraggable(handle, target, library)
    ...
    local connectionHost = library or {
...
function Library:CreateWindow(options)
    ...
    local brandTitle = create("TextLabel", {
        ...
        Text = options.Name or "Saloi Hub",
        TextColor3 = Theme.Text,
        TextSize = 24,
        Font = Fonts.Title,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    local brandSubtitle = create("TextLabel", {
        ...
        Text = options.Subtitle or "Custom local UI",
        TextColor3 = Theme.Muted,
        TextSize = 13,
        Font = Fonts.Body,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    self:Connect(tabLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
        setCanvasToLayout(tabScroller, tabLayout, 12)
    end)
    ...
    local sidebarFooter = create("TextLabel", {
        ...
        Text = string.format("Player: %s", LocalPlayer and LocalPlayer.Name or "Unknown"),
        TextColor3 = Theme.Muted,
        TextSize = 12,
        Font = Fonts.Body,
        TextWrapped = true,
    ...
    local window = setmetatable({
        ...
        Tabs = {},
        ActiveTab = nil,
        WindowName = options.Name or "Saloi Hub",
        WindowSubtitle = options.Subtitle or "Custom local UI",
    }, WindowClass)

    self.Window = window

    closeButton.MouseEnter:Connect(function()
        tween(closeButton, { BackgroundColor3 = Theme.Accent })
    end)

    closeButton.MouseLeave:Connect(function()
        tween(closeButton, { BackgroundColor3 = Theme.Inline })
    end)
    ...
    local function updateScale()
        ...
        if viewport.X < 1000 then
            rootScale.Scale = math.clamp(viewport.X / 980, 0.72, 1)
        else
            rootScale.Scale = 1
        end
    end
    ...
    if Workspace.CurrentCamera then
        self:Connect(Workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"), updateScale)
    end
...
function WindowClass:SelectTab(targetTab)
    for _, tab in ipairs(self.Tabs) do
        ...
        tween(tab.ButtonStroke, {
            Transparency = isActive and 0 or 0.35,
            Color = isActive and Theme.AccentSoft or Theme.Border,
        })
        tween(tab.ButtonLabel, {
            TextColor3 = isActive and Theme.Background or Theme.Text,
        })
        tween(tab.ButtonShadow, {
...
function WindowClass:CreateTab(title)
    ...
    self.Library:Connect(pageLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
        setCanvasToLayout(page, pageLayout, 16)
    end)
    ...
    local tab = setmetatable({
        ...
        ButtonShadow = tabButtonShadow,
        ButtonLabel = tabButtonLabel,
        Page = page,
        Layout = pageLayout,
    }, TabClass)
    ...
    tabButton.MouseEnter:Connect(function()
        if self.ActiveTab ~= tab then
            tween(tabButton, { BackgroundColor3 = Theme.Inline })
        end
    end)
...
function TabClass:CreateButton(data)
    ...
    button.MouseEnter:Connect(function()
        tween(card, { BackgroundColor3 = Theme.Inline })
    end)

    button.MouseLeave:Connect(function()
        tween(card, { BackgroundColor3 = Theme.Surface })
    end)
...
function TabClass:CreateToggle(data)
    ...
    if data.Flag then
        self.Window.Library.Flags[data.Flag] = currentValue
    end
    ...
    local stateLabel = create("TextLabel", {
        ...
        Text = currentValue and "Enabled" or "Disabled",
        TextColor3 = Theme.Muted,
        TextSize = 12,
        Font = Fonts.Body,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    local knob = create("Frame", {
        Parent = track,
        Position = currentValue and UDim2.new(1, -22, 0.5, -9) or UDim2.new(0, 4, 0.5, -9),
    ...
    local function setValue(value, shouldTrigger)
        ...
        if shouldTrigger and data.Callback then
            task.spawn(function()
...
function TabClass:CreateDropdown(data)
    ...
    if not contains(options, selected) then
        selected = options[1]
    end

    if data.Flag then
        self.Window.Library.Flags[data.Flag] = selected
    end
        ...
        for _, optionValue in ipairs(options) do
            ...
            optionButton.MouseEnter:Connect(function()
                if optionValue ~= selected then
                    tween(optionButton, { BackgroundColor3 = Theme.Inline })
                end
            end)
    ...
    local function refresh(newOptions, keepSelection)
        ...
        if not keepSelection or not contains(options, selected) then
            selected = options[1]
        end
    ...
    function controller:Refresh(newOptions, keepSelection)
        refresh(newOptions, keepSelection)
    end
...
function TabClass:CreateSlider(data)
    ...
    if data.Flag then
        self.Window.Library.Flags[data.Flag] = value
    end
...
local Window = Library:CreateWindow({
    Name = "Saloi Hub",
    Subtitle = "Auto Farm • Event • Teleport",
})
...
HomeTab:CreateParagraph({
    Title = "Saloi Hub",
    Content = "Da doi sang custom local UI hoan toan. Hub nay se tu load 3 module trong folder: Autofarm, Event va Teleport Island.",
})

HomeTab:CreateSection("Thông tin")
HomeTab:CreateLabel("UI: Custom local library")
...
HomeTab:CreateButton({
    ...
    Callback = function()
        Hub.Helpers.Notify(
            "Saloi Hub",
            "Da nap " .. tostring(loadedCount) .. "/" .. tostring(#modules) .. " module.",
            5
        )
    end,
})
...
Hub.Helpers.Notify(
    "Saloi Hub",
    "Khoi dong xong. Da nap " .. tostring(loadedCount) .. "/" .. tostring(#modules) .. " module.",
    5
)
